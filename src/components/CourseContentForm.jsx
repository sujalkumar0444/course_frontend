...
export default function CourseContentForm({ moduleId }) {
  console.log(moduleId);
    const editor = useRef(null);
  const [content, setContent] = useState("");
  // const [module_id, setModuleId] = useState(""); // State to hold module_id
  const [lesson_title, setLessonTitle] = useState(""); // State to hold lesson_title
  const handleCreateContent = async () => {
    try {
        const response = await axios.post('http://localhost:8800/add/content', {
            module_id:moduleId,
            lesson_title,
